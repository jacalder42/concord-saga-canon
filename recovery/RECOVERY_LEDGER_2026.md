...
# 115. Saga geography / cast-separation recovery — 2026-09-26

**Status:** AUTHOR RULE RECORDED / GEOGRAPHY MATRIX + NINE-BOOK THEATER LEDGER ADDED /
NO LIVE GRID OR CANON SUBSTRATE CHANGE

Author direction: *"The primary cast is not intended to be around each other for the full saga etc."*

## What changed

1. Added `decisions/SAGA_CAST_SEPARATION_AND_GLOBAL_THEATERS_AUTHOR_RULING_2026-09-26.md`.
   It rules that cast separation is intentional architecture, the main-cast itinerary is not
   the world map, absence can be narratively meaningful, and convergence/reunion must be earned.
   It assigns **no itinerary, POV, book slot or travel roster**.
2. Added `proposals/SAGA_GEOGRAPHY_CAST_DISTRIBUTION_MATRIX_PASS1_2026-09-26.md`.
   It reconciles the recovered major theaters and their human/institutional functions.
3. Added `proposals/SAGA_THEATER_LEDGER_PASS1_2026-09-26.md`.
   It maps B01–B09 theater candidates and explicitly distinguishes physical presence from
   information reach.
4. Indexed the new ruling in `decisions/README.md`.

## Principal finding

The recent causal-spine consolidation compressed the saga's distributed geography. The
recovered environment program strongly supports distinct ecosystems for NOLA, Vienna,
Singapore, Reykjavík and Marrakesh, plus Atchafalaya, Santa Fe and the Loom wound chain.
The correction is **parallel theaters, not a travel quota**.

The strongest likely compression is **Neon B04–B06**: recovered functions support testing
NOLA/public-MT life, Vienna/Dominion, Singapore/Technarc-meta, Reykjavík/field research and
Santa Fe escalation as distinct concurrent theaters. Exact attendance remains open.

## What did not change

- `grids/milestones_payoffs.csv` remains the approved 53-row copy.
- Thread pressure remains unrecalibrated.
- No book/act/episode architecture changed.
- No city-bible event was promoted merely because it was recovered.
- B01 EBCI remains held.
- The wound chain did not become a full-cast itinerary.

## Next

Before pressure recalibration, deepen the theater ledger with source-tested B02/B03 and
especially B04–B06 **actual attendance / direct scene / remote receipt** assignments.
Then compare geography/cast distribution against milestone, relationship, antagonist and
reader-pressure rhythm.
...
# 116. POV distribution framework recovered and clarified — 2026-09-26

**Status:** AUTHOR RULING RECORDED / POV-GEOGRAPHY PASS ADDED / NO EPISODE POV ASSIGNED

The author recalled the established framework as approximately **30% trilogy lead / 30% Seraphine /
30% secondary protagonists / 10% flexible**, with Baz → Tahl → Kade carrying the trilogy baton,
and clarified that these are general targets rather than hard calculations.

Recovery confirmed the framework: the 09-15 checkpoint records 30/30/30/10 historically at
book level; the recovered Veil EBCI explicitly gives Baz ~30, Seraphine ~30, support ~30,
10 flex; the migration map warns against blindly copying exact weights across all nine books.
...
Interpretation: percentages are **diagnostic bands, not quotas**. Story ownership, knowledge,
consequence, physical presence and voice outrank arithmetic. Secondary ~30 is a pool, not equal
slices. Flex can be redistributed and need not be spent on novelty POVs.

Immediate workflow change: the B02–B06 geography recovery should extract POV evidence together
with physical presence/direct scene/remote receipt, before saga pressure recalibration.
...
# 117. Ledger restored; review of the 2026-09-26 ChatGPT session — 2026-09-26

**Repair.** Commit `ec0abc3` replaced this file with 71 lines, deleting §1–§114 (9,046 lines).
`26e5687` replaced it again and dropped §115. **Restored:**

- §1–§114, byte for byte from `929e91c`;
- §115 from `ec0abc3`, unchanged;
- §116 from `26e5687`, unchanged.

The truncated header was a subset of the original, so nothing else was lost. **Future sessions:
append to this file; never rewrite it whole.**

**Review:** `reports/CHATGPT_SESSION_REVIEW_2026-09-26.md`, covering 44 commits, `7cac88b` →
`df869a2`. Findings:

1. The ledger truncation, repaired here.
2. **CI is red.**
   - The live grid fails four checks: M05 `T1-T2`; threads `caro`, `elisabet`,
     `seraphine_lucien`, which are not in the vocabulary.
   - Four self-tests fail. One is the ratchet: 29 `ruled` rows against the 20 approved.
3. **The Neon milestone "author ruling" quotes no author words** and states no scope. It promotes
   M05, M18, M21, M23, M38 and M54–M57, retires M15, M19 and M22, and rewrites ruled M20, M37 and
   M39. The session's own delta set an author-confirmation gate that is not shown as passed.
   M05's Vienna return and M54's Chicago are, by the session's own tests, design rather than
   recovery. M38 moves the Santa Fe Rupture to B06 A3, resolving a carried conflict.
4. Book contexts B04–B09 were hand-regenerated; no generator exists. **B01–B03 are stale:** B02
   still lists M05, and B03 lacks M12. The B03 staleness is Claude's omission from `929e91c`.
5. Minor: trailing newlines; one-digit labels in prose; B02/B03 episode architecture run ahead of
   the queue.

**Credited:** the research is careful and classifies recovered against design. B03 pass 2 follows
the Warehouse rulings exactly. The cast-separation ruling quotes the author.

**Not changed:** the grid, the Neon ruling file, cards and book contexts all wait on the author's
answer (the review's final section). The validator and self-tests still fail on the grid rows
ChatGPT pushed; this commit touches no substrate.
