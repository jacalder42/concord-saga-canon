...
- **Additive changes are approved; destructive changes need a specific author ruling**
  (author, 2026-09-26; applies to every assistant). Destructive includes deleting or truncating,
  overwriting rather than appending, changing a status to `ruled` or `retired`, rewriting a ruled
  row or ruling, and editing cards or rules against their content. Adding files, rows and ledger
  entries, and regenerating derived blocks with the tool, are additive.
...
cite.** **"Proceed" approves the immediately preceding proposed development or changes** (author,
2026-09-26). It is an approval, not a weak acceptance, and it covers that proposal only: design
choices made afterwards while proceeding need their own approval. An approved development is Tier B
(§5); promoting a row or card to ruled canon still needs a specific ruling
(`decisions/ASSISTANT_CHANGE_AUTHORITY_AND_PROCEED_AUTHOR_RULING_2026-09-26.md`). Record every acceptance as a ruling in `decisions/` that states what was accepted
...
| B | Explicitly approved development outputs, including those approved with "Proceed" (ruled 2026-09-26) |
...
- **Validation:** canon scope **0** violations; **144** self-tests; source verifier PASS at
  138 files. All-scope carries **356** `CHK_RETIRED_TERMS` notices and 112 `CHK_SID_FORMAT`
...
- **Saga structure:** event tiers A–D and the unslotted pool
  ...
  grid was a candidate set, not the saga skeleton. B08 now has ruled turns (grid M44–M47) from the
  Loom spine; POOL-017 stays available as pressure (saga pass 1, ledger §85).
